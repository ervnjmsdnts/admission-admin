...
export default function ApproveExamDialog({
  ...
  const onApproveAdmission = async () => {
    ...
    try {
      setIsLoading(true);
      await fetch('/api/send-approval-examination', {
        method: 'POST',
        headers: {
      ...
      await updateDoc(docRef, { status: 'approvedExamination' });
      toast({ title: 'Examination approved' });
      onClose();
    } catch (error) {
      toast({ title: `Approving examination failed`, variant: 'destructive' });
    } finally {
      setIsLoading(false);
    }
  };
